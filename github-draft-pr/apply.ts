...
/** Apply an approved same-repository draft plan; never merges or updates a PR. */
export async function applyDraft(
  ...
  const packet = await requireDraftPacket(rawPacket);
  const approved = DraftPlanSchema.parse(rawPlan);
  if (
  ...
  const current = await planDraft(config, packet, fetchImpl);
  ...
  if (!response.ok) {
    if (response.status === 422) {
      ...
      const raced = await planDraft(config, packet, fetchImpl);
    ...
    throw new Error(
      `GitHub draft PR creation failed with HTTP ${response.status}`,
    );
  }
