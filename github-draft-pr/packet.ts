...
/** Reject malformed, tampered, unready, or non-draft packets before GitHub I/O. */
export async function requireDraftPacket(raw: unknown): Promise<Packet> {
  ...
  if (!packet.submission.draft) {
    throw new Error(
      "GitHub draft-PR adapter requires submission.draft to be true",
    );
  }
  return packet;
}
